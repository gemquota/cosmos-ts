/**
 * Dashboard server for RSIS telemetry and reporting.
 *
 * TypeScript port of rsis/dashboard/app.py (FastAPI → Node.js http).
 * Serves HTML dashboard and JSON API endpoints for KG stats, velocity, trends.
 */
...
import type { DataPoint } from './extrapolation.js';
...
export class RSISDashboard {
  ...
  private fallbackHtml(vars: Record<string, unknown>): string {
    return `<!DOCTYPE html>
<html><head><title>RSIS Dashboard</title></head>
<body>
  <h1>RSIS Dashboard</h1>
  ...
  start(): Promise<void> {
    return new Promise((resolve) => {
      ...
      this.server.listen(this.port, this.host, () => {
        console.log(
          `RSIS Dashboard listening on http://${this.host}:${this.port}`,
        );
        resolve();
      });
    });
